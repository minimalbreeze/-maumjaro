...
> 이 파일은 `node scripts/utm.mjs`가 만든다. 손으로 고치면 덮어써진다.

글을 올릴 때 **아래 링크를 그대로 복사해서** 쓰세요. 주소가 길어 보여도 그대로 붙여야
나중에 어떤 종류의 글이 사람을 데려왔는지 갈라서 볼 수 있습니다.

링크는 반드시 **실제 하이퍼링크로** 걸어주세요. 텍스트로만 두면 클릭이 안 되고
백링크로도 안 잡힙니다.
...
| 값 | 종류 | 어떤 글인가 |
|---|---|---|
| `empathy` | 감정 공감형 | "괜찮은 척하기 지친 날"처럼 상황을 먼저 말하는 글 |
| `checklist` | 체크리스트형 | "오늘 이런 사람은 들어오세요" + 해당 항목 나열 |
| `fortune` | 운세형 | 오늘의 맘운 / 띠·별자리 운세 |
| `tarot` | 타로형 | 오늘의 타로 3장 |
| `quiz` | 참여형 | "오늘 당신의 마음은 몇 번?" 번호 고르기 |
| `sports` | 스포츠 멘탈 | 응원팀 진 날 감정 정리 |
| `diet` | 다이어트 멘탈 | 한 끼 망쳤을 때 / 정체기 |
...
| 글의 종류 | 붙여넣을 링크 |
|---|---|
| 감정 공감형 | `https://maumjaro.minimalbreeze.com/?utm_source=threads&utm_medium=social&utm_content=empathy` |
| 체크리스트형 | `https://maumjaro.minimalbreeze.com/?utm_source=threads&utm_medium=social&utm_content=checklist` |
| 운세형 | `https://maumjaro.minimalbreeze.com/?utm_source=threads&utm_medium=social&utm_content=fortune` |
| 타로형 | `https://maumjaro.minimalbreeze.com/?utm_source=threads&utm_medium=social&utm_content=tarot` |
| 참여형 | `https://maumjaro.minimalbreeze.com/?utm_source=threads&utm_medium=social&utm_content=quiz` |
| 스포츠 멘탈 | `https://maumjaro.minimalbreeze.com/?utm_source=threads&utm_medium=social&utm_content=sports` |
| 다이어트 멘탈 | `https://maumjaro.minimalbreeze.com/?utm_source=threads&utm_medium=social&utm_content=diet` |
...
**보고서 → 획득 → 트래픽 획득**으로 들어가서, 표 왼쪽 위의 차원 드롭다운을
`세션 수동 광고 콘텐츠`(Session manual ad content)로 바꾸면 `utm_content` 값별로
갈라져 보입니다. 채널까지 같이 보려면 그 옆 `+`를 눌러 `세션 소스`를 더하세요.

### 숫자를 읽을 때 주의할 것

- **유형 하나당 최소 5~7회**는 올려야 비교가 됩니다. 1~2회 올린 숫자는 그날 요일·시간대
  차이에 묻혀서 의미가 없습니다.
- 요일을 돌려가며 다른 유형을 올리면 **요일 효과와 유형 효과가 섞여** 분리가 안 됩니다.
  한 유형을 여러 번 반복한 뒤 다음 유형으로 넘어가세요.
- 내 방문은 홈에서 `?noga=1`을 한 번 열어 빼두세요.

## 앱 안에서 공유된 링크는 따로 잡힙니다

사용자가 앱의 공유 버튼으로 내보낸 링크에는 `share-utm.js`가 `utm_source=share`를
붙입니다. 위 표의 링크(`naver`/`threads`/`tistory`)와 섞이지 않으니,
**내가 올린 글로 온 사람**과 **사용자가 퍼뜨려서 온 사람**을 구분해서 볼 수 있습니다.
