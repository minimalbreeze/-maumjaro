/* ============================================================================
   stock.js — 블루칩 나침반 화면 로직
   ----------------------------------------------------------------------------
   구조:
     state  ─ 시장(국내/미장), 국면 다이얼, 나의 성향, 필터. localStorage에 저장.
     render ─ 상태가 바뀌면 해당 뷰만 다시 그린다. 프레임워크 없음(빌드 도구 금지).

   저장 키는 전부 `maumjaro:stock:` 접두사를 쓴다. 맘운자로 앱이 쓰는 기존 키와
   절대 겹치지 않는 별도 네임스페이스다 — 이 사이트는 기존 데이터를 읽지도
   지우지도 않는다.
   ========================================================================== */
...
(function () {
  ...
  var D = window.StockGuideData;
  var M = window.StockMarketContext;
  var KEY = 'maumjaro:stock:';
  ...
  function renderLearn() {
    ...
    h.push('<div class="sec"><div class="sec-head"><h2>🔁 값을 되돌리기</h2></div><div class="card">' +
      '<div class="slot-d">국면 다이얼과 성향을 처음 상태로 되돌립니다. 이 사이트가 저장한 값만 지우며, ' +
      '<b>다른 서비스의 기록에는 손대지 않습니다.</b></div>' +
      '<button class="btn danger" id="reset" style="margin-top:12px">이 사이트 설정 초기화</button></div></div>');
